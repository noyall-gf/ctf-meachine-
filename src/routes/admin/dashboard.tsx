...
function AdminDashboard() {
  ...
  const [panelFlag, setPanelFlag] = useState<string | null>(null);
  const [showPanelFlag, setShowPanelFlag] = useState(true);

  useEffect(() => {
    void fetch("/api/admin/flag?name=admin-panel", { credentials: "include" })
      .then((response) => response.json())
      .then((result: { success?: boolean; flag?: string }) => {
        if (result.success && result.flag) setPanelFlag(result.flag);
      })
      .catch(() => undefined);
  ...
  return (
    <div className="min-h-screen bg-background px-4 py-8">
      {showPanelFlag && panelFlag && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4"
          role="dialog"
          aria-modal="true"
          aria-labelledby="admin-panel-flag-title"
        >
          <div className="w-full max-w-lg rounded-xl border border-primary/30 bg-card p-6 shadow-xl">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">
              ShopNest
            </p>
            <h2 id="admin-panel-flag-title" className="mt-2 text-2xl font-bold">
            ...
            <button
              type="button"
              onClick={() => setShowPanelFlag(false)}
