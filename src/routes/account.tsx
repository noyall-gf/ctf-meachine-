...
function AccountPage() {
  ...
  const [flag, setFlag] = useState<string | null>(null);
    ...
    useEffect(() => {
      if (!user || !isCarlos) return;

      void fetch(`/api/user?id=${user.id}`)
        .then((response) => response.json())
        .then((result: { accountFlag?: string }) => {
          if (result.accountFlag) setFlag(result.accountFlag);
        })
        .catch(() => undefined);
    }, [isCarlos, user?.id]);
